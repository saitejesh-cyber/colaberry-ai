          ...
          <div className="surface-panel section-shell p-6">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
              {(Object.keys(summaryByEntity || {}) as Array<Exclude<EntityKey, "all">>).map((key) => {
                ...
                return (
                  <article key={key} className="rounded-lg border border-slate-200/80 bg-slate-50/80 p-4 dark:border-slate-700/70 dark:bg-slate-900/70">
                    <p className="text-label font-semibold uppercase tracking-[0.18em] text-slate-500">{key}</p>
                    <p className="mt-2 text-xl font-semibold text-slate-900 dark:text-slate-100">{value.total}</p>
          ...
          <div className="surface-panel section-shell p-6">
            ...
            {worstRows.length === 0 ? (
              ...
              <div className="mt-5 overflow-x-auto">
                <table className="w-full min-w-[980px] border-separate border-spacing-y-2 text-sm" aria-label="Catalog health data">
                  <thead>
                    <tr className="text-left text-label font-semibold uppercase tracking-[0.18em] text-slate-500">
                  ...
                  <tbody>
                    {worstRows.map((row) => (
                      <tr key={`${row.entity}-${row.id}`} className="rounded-lg bg-slate-50/80 dark:bg-slate-900/70">
                        <td className="px-3 py-3 text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">
                          {row.entity}
                        </td>
                        ...
                        <td className="px-3 py-3">
                          <span className="inline-flex rounded-md border border-slate-300/80 px-2 py-1 text-xs font-semibold text-slate-700 dark:border-slate-700 dark:text-slate-200">
                        ...
                        <td className="px-3 py-3 text-xs text-slate-600 dark:text-slate-300">{formatDate(row.updatedAt)}</td>

...
export const getServerSideProps: GetServerSideProps = async ({ res }) => {
  ...
  for (const a of agents as any[]) {
    ...
    lines.push(`- ${name} | ${SITE}/aixcelerator/agents/${a.slug} | ${desc}`);
  }

  lines.push("", `## MCP Servers (${mcpServers.length})`, "");
  for (const m of mcpServers as any[]) {
    if (!m.slug) continue;
    const name = m.name || m.title || "Untitled";
    const desc = (m.description || m.summary || "").replace(/\n/g, " ").slice(0, 200);
    lines.push(`- ${name} | ${SITE}/aixcelerator/mcp/${m.slug} | ${desc}`);
  }
  ...
  for (const s of skills as any[]) {
    ...
    lines.push(`- ${name} | ${SITE}/aixcelerator/skills/${s.slug} | ${desc}`);
  }

  lines.push("", `## Use Cases (${useCases.length})`, "");
  for (const u of useCases as any[]) {
    if (!u.slug) continue;
    const name = u.name || u.title || "Untitled";
    const desc = (u.description || u.summary || "").replace(/\n/g, " ").slice(0, 200);
    lines.push(`- ${name} | ${SITE}/use-cases/${u.slug} | ${desc}`);
  }

  lines.push("", `## Podcast Episodes (${podcasts.length})`, "");
  for (const p of podcasts as any[]) {
    if (!p.slug) continue;
    const title = p.title || "Untitled";
    const desc = (p.description || p.summary || "").replace(/\n/g, " ").slice(0, 200);
    lines.push(`- ${title} | ${SITE}/resources/podcasts/${p.slug} | ${desc}`);
  }
  ...
  for (const a of articles as any[]) {
    ...
    const desc = (a.description || a.summary || "").replace(/\n/g, " ").slice(0, 200);
    lines.push(`- ${title} | ${SITE}/resources/articles/${a.slug} | ${desc}`);
  }

    ...
    <Layout>
      ...
      <section className="relative overflow-hidden rounded-2xl" style={{ background: "var(--gradient-hero)" }}>
        ...
        <div className="relative z-10 grid gap-7 px-5 py-11 sm:px-8 sm:py-13 lg:grid-cols-[1.1fr_0.9fr] lg:items-center lg:px-12 lg:py-16">
          ...
          {/* Hero product image */}
          <div className="rise-in hidden lg:block" style={{ animationDelay: "0.4s" }}>
            <div className="card-feature relative overflow-hidden rounded-xl border border-[var(--stroke)] shadow-md">
              <Image
                src={heroImage("hero-platform-cinematic.webp")}
                alt="Colaberry AI platform dashboard"
                width={720}
                height={405}
                quality={90}
                priority
      ...
      <section className="reveal section-spacing surface-panel p-5 sm:p-6">
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {["Slack", "Microsoft Teams", "Google Drive", "Salesforce", "ServiceNow", "Workday", "Jira", "Okta", "Zendesk", "Snowflake", "AWS", "GitHub"].map((name) => (
            <span key={name} className="chip chip-neutral rounded-md px-3 py-1.5 text-xs font-medium">
              {name}
            </span>
          ))}
        </div>
...
function CatalogCard({
  ...
  return (
    <Link
      href={href}
      className="card-glass group flex h-full min-h-[268px] flex-col overflow-hidden p-0"
      ...
      <div className="media-premium-frame border-0 border-b border-slate-200/80 rounded-none">
        <div className="relative aspect-[16/10] w-full">
          ...
          <div className="absolute left-3 top-3">
            <div className="chip chip-muted rounded-md border border-slate-200/80 bg-white/90 px-2.5 py-1 text-xs font-semibold text-slate-700 dark:border-slate-600 dark:bg-slate-900/90 dark:text-slate-200">
              {meta}
            </div>
          </div>
        </div>
  ...
  return (
    ...
    >
      {episode.coverImageUrl ? (
        ...
        <div className="flex h-32 w-full shrink-0 items-center justify-center rounded-lg bg-slate-100 dark:bg-slate-800 sm:h-24 sm:w-40">
      ...
      <span className="hidden shrink-0 text-slate-400 transition-transform group-hover:translate-x-1 group-hover:text-slate-600 dark:group-hover:text-slate-300 sm:block" aria-hidden="true">
        →
      </span>
    </Link>
  );
            ...
            return (
              <li key={episode.slug}>
                ...
                >
                  ...
                  <span className="ml-1 shrink-0 text-slate-400 transition-transform group-hover:translate-x-0.5 group-hover:text-slate-600 dark:group-hover:text-slate-300 dark:text-slate-500">
                    →
                  </span>
                </Link>
              </li>
...
function AgentRail({
  ...
  return (
    <article className="section-card rounded-lg p-5">
      ...
      <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">{description}</p>
      {items.length === 0 ? (
        <p className="mt-3 text-sm text-slate-500 dark:text-slate-400">Agent signals will appear after next refresh.</p>
      ) : (
        <ul className="mt-4 grid gap-2">
          {items.map((agent) => (
            <li key={agent.slug || agent.id}>
              ...
              >
                ...
                <span className="shrink-0 text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300 dark:text-slate-400">
                  {detailType === "latest"
...
function SkillRail({
  ...
  return (
    <article className="section-card rounded-lg p-5">
      ...
      {items.length === 0 ? (
        <p className="mt-3 text-xs text-slate-500 dark:text-slate-400">Skill signals will appear after next refresh.</p>
      ) : (
...
function UseCaseRail({
  ...
  return (
    <article className="section-card rounded-lg p-5">
      ...
      <p className="mt-1 text-xs text-slate-600 dark:text-slate-400">{description}</p>
      {items.length === 0 ? (
        <p className="mt-3 text-xs text-slate-500 dark:text-slate-400">Use case signals will appear after next refresh.</p>
      ) : (
        <ul className="mt-3 grid gap-2">
          {items.map((useCase) => (
...
function McpRail({
  ...
  return (
    <article className="section-card rounded-lg p-5">
      ...
      <p className="mt-1 text-xs text-slate-600 dark:text-slate-400">{description}</p>
      {items.length === 0 ? (
        <p className="mt-3 text-xs text-slate-500 dark:text-slate-400">MCP signals will appear after next refresh.</p>
      ) : (
        <ul className="mt-3 grid gap-2">
          {items.map((mcp) => (
...
function QuickLink({
  ...
  const content = (
    <div className="flex items-start justify-between gap-3">
      ...
      <div className="mt-0.5 text-slate-400 transition-transform group-hover:translate-x-0.5 group-hover:text-slate-600 dark:group-hover:text-slate-300 dark:text-slate-500">
        <span aria-hidden="true">→</span>
      </div>
    </div>
  );
  ...
  if (external) {
    return (
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="card-glass group flex items-start justify-between gap-3 p-5"
        aria-label={`Open ${title}`}
      >
        {content}
      </a>
...
function IndustryTile({
  ...
  return (
    ...
    >
      <div className="absolute right-4 top-4 text-slate-400 transition-transform group-hover:translate-x-0.5 group-hover:text-slate-600 dark:group-hover:text-slate-300">
        <span aria-hidden="true">→</span>
      </div>
      <div className="flex h-14 w-14 items-center justify-center rounded-lg border border-slate-200/40 bg-slate-50 text-slate-600 dark:border-slate-700/20 dark:bg-slate-800/40 dark:text-slate-300">
...
function FeatureCard({
  ...
  return (
    <article className="card-glass p-6">
      <h3 className="text-caption font-semibold text-slate-900 dark:text-slate-100">
        {title}
      </h3>

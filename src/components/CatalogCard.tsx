...
export type CatalogVariant = "agent" | "mcp" | "skill";
...
const VARIANT_CONFIG: Record<CatalogVariant, { label: string; basePath: string; fallbackDescription: string }> = {
  agent: {
    label: "Agent",
    basePath: "/aixcelerator/agents",
    fallbackDescription: "Structured profile and delivery context available.",
  },
  mcp: {
    label: "MCP",
    basePath: "/aixcelerator/mcp",
    fallbackDescription: "Structured MCP server profile and integration metadata.",
  },
  skill: {
    label: "Skill",
    basePath: "/aixcelerator/skills",
    fallbackDescription: "Structured skill profile and usage context available.",
  },
};
...
export default function CatalogCard({ item, variant }: CatalogCardProps) {
  const config = VARIANT_CONFIG[variant];
  const href = item.slug ? `${config.basePath}/${item.slug}` : config.basePath;
  const statusTone = getStatusTone(item.status);
  const statusLabel = capitalizeFirst(item.status);
  const summary = item.description || config.fallbackDescription;
  const industry = item.industry || "General";
  const ratingLabel = typeof item.rating === "number" ? item.rating.toFixed(1) : null;
  const usageLabel =
    typeof item.usageCount === "number" && item.usageCount > 0 ? formatUsage(item.usageCount) : null;
  const lastUpdatedLabel = formatShortDate(item.lastUpdated);
  ...
  if (item.source) metaParts.push(capitalizeFirst(item.source));

  return (
    <Link href={href} className="group block" aria-label={`View ${config.label.toLowerCase()} ${item.name} details`}>
      <div className="catalog-card p-5">
        ...
        <div className="mt-3">
          <h3 className="truncate text-caption font-semibold text-[var(--text-heading)] dark:text-slate-100">{item.name}</h3>

...
| `caption` | 0.9375rem | Source Sans | Card titles, subtitles, descriptions |
...
**Banned patterns** — never use arbitrary bracket font sizes:
- `text-[Npx]` or `text-[Nrem]` — always use named tokens from the scale above
- If a new size is genuinely needed, add it to `tailwind.config.ts` fontSize first

**Heading hierarchy per page type:**
| Page type | H1 | H2 (sections) | H3 (cards) |
|-----------|-----|---------------|------------|
| Homepage | `display-md / lg / xl` (responsive) | Via `SectionHeader` | `text-caption` |
| Catalog/index | Via `EnterprisePageHero` | Via `SectionHeader` | `text-caption` or `text-lg` |
| Detail pages | `display-sm / md` | `display-xs` | `text-caption` |
| Content (articles) | `display-sm / md` | Standard Tailwind `text-xl` | — |

**Card title scale:**
- Small cards (compact grids): `text-caption` (15px)
- Medium cards (featured): `text-lg` (18px)
- Hero/featured items: `text-display-xs` (24px)

**Metadata scale:**
- Inline dates, counts, descriptions: `text-xs` (12px)
- Uppercase kickers and section labels: `text-label` (11px, tracking 0.14em)
...
| `rgba(250,249,246,0.92)` | `rgba(15,27,48,0.86)` | Section shells (glass) |
